...
import cn.edu.scut.sse.supply.dao.DownstreamEnterpriseContractDAO;
...
public class DownstreamEnterpriseService {
    ...
    public ResponseResult receiveContract(String token, int fid) {
        ...
        DownstreamEnterpriseContract contract = new DownstreamEnterpriseContract();
        ...
        try {
            return downstreamEnterpriseContractDAO.receiveContractToFisco(fid, signature);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseResult().setCode(-11).setMsg("内部状态错误");
        }
