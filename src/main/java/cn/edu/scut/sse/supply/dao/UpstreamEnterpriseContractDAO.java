...
import cn.edu.scut.sse.supply.pojo.UpstreamEnterpriseContract;
...
@Repository
public class UpstreamEnterpriseContractDAO {
    ...
    public void saveContract(UpstreamEnterpriseContract contract) {
    ...
    public void updateContract(UpstreamEnterpriseContract contract) {
        Session session = SessionFactoryUtil.getSessionFactoryInstance().openSession();
        session.beginTransaction();
    ...
    public UpstreamEnterpriseContract getContract(int fid) {
        SessionFactory sessionFactory = SessionFactoryUtil.getSessionFactoryInstance();
        Session session = sessionFactory.openSession();

        CriteriaBuilder criteriaBuilder = session.getCriteriaBuilder();
        CriteriaQuery<UpstreamEnterpriseContract> criteriaQuery = criteriaBuilder.createQuery(UpstreamEnterpriseContract.class);
        Root<UpstreamEnterpriseContract> root = criteriaQuery.from(UpstreamEnterpriseContract.class);
        criteriaQuery.select(root);
        criteriaQuery.where(criteriaBuilder.equal(root.get("fid"), fid));

        UpstreamEnterpriseContract contract = session.createQuery(criteriaQuery).uniqueResult();

        session.close();
        return contract;
    }

    public List<UpstreamEnterpriseContract> listEnableContract() {
        SessionFactory sessionFactory = SessionFactoryUtil.getSessionFactoryInstance();
        Session session = sessionFactory.openSession();
        ...
        List<UpstreamEnterpriseContract> contracts = session.createQuery(criteriaQuery).list();

        session.close();
        return contracts;
    }

    public List<UpstreamEnterpriseContract> listRecycleContract() {
        SessionFactory sessionFactory = SessionFactoryUtil.getSessionFactoryInstance();
        Session session = sessionFactory.openSession();
    ...
    public ResponseResult saveContractToFisco(UpstreamEnterpriseContract upstreamEnterpriseContract, String sponsorSignature) throws Exception {
        Web3j web3j = Web3jUtil.getWeb3j();
        ContractRepo contractRepo = ContractRepo.load(address, web3j, credentials, new StaticGasProvider(gasPrice, gasLimit));

        TransactionReceipt receipt = contractRepo.launchContract(BigInteger.valueOf(upstreamEnterpriseContract.getFid()),
                upstreamEnterpriseContract.getHash(), BigInteger.valueOf(upstreamEnterpriseContract.getSponsor()),
                BigInteger.valueOf(upstreamEnterpriseContract.getReceiver()), sponsorSignature).send();
