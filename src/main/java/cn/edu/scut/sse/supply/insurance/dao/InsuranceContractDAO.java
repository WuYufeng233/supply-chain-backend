...
public class InsuranceContractDAO {
    ...
    private final int ENTERPRISE_CODE = 3001;

    public void saveContract(InsuranceContract contract) {
    ...
    public void updateContract(InsuranceContract contract) {
        Session session = SessionFactoryUtil.getSessionFactoryInstance().openSession();
        session.beginTransaction();
    ...
    public InsuranceContract getContract(int fid) {
        SessionFactory sessionFactory = SessionFactoryUtil.getSessionFactoryInstance();
        Session session = sessionFactory.openSession();

        CriteriaBuilder criteriaBuilder = session.getCriteriaBuilder();
        CriteriaQuery<InsuranceContract> criteriaQuery = criteriaBuilder.createQuery(InsuranceContract.class);
        Root<InsuranceContract> root = criteriaQuery.from(InsuranceContract.class);
        criteriaQuery.select(root);
        criteriaQuery.where(criteriaBuilder.equal(root.get("fid"), fid));

        InsuranceContract contract = session.createQuery(criteriaQuery).uniqueResult();

        session.close();
        return contract;
    }

    public List<InsuranceContract> listEnableContract() {
        SessionFactory sessionFactory = SessionFactoryUtil.getSessionFactoryInstance();
        Session session = sessionFactory.openSession();
        ...
        List<InsuranceContract> contracts = session.createQuery(criteriaQuery).list();

        session.close();
        return contracts;
    }

    public List<InsuranceContract> listRecycleContract() {
        SessionFactory sessionFactory = SessionFactoryUtil.getSessionFactoryInstance();
        Session session = sessionFactory.openSession();
    ...
    public ResponseResult saveContractToFisco(InsuranceContract insuranceContract, String sponsorSignature) throws Exception {
        Web3j web3j = Web3jUtil.getWeb3j();
        ContractRepo contractRepo = ContractRepo.load(address, web3j, credentials, new StaticGasProvider(gasPrice, gasLimit));

        BigInteger contractId = new BigInteger(String.valueOf(ENTERPRISE_CODE).concat(String.valueOf(insuranceContract.getFid())));
        TransactionReceipt receipt = contractRepo.launchContract(contractId,
                insuranceContract.getHash(), BigInteger.valueOf(insuranceContract.getSponsor()),
                BigInteger.valueOf(insuranceContract.getReceiver()), sponsorSignature).send();
